...
bool BuildInputLayout(const AVFrame &frame,
    ...
    if (av_channel_layout_check(&frame.ch_layout) != 0) {
        ...
        if (copy_result < 0) {
            if (error_message != nullptr) {
                *error_message = QStringLiteral("failed to copy ASR input channel layout: %1")
...
bool PcmConverter::ConvertFrame(const AVFrame &frame, PcmChunk *chunk, QString *error_message) {
    ...
    const int output_samples = static_cast<int>(
        av_rescale_rnd(delay + frame.nb_samples, kAsrSampleRate, frame.sample_rate, AV_ROUND_UP));
    if (output_samples <= 0) {
        return true;
    }
    ...
    if (converted_samples < 0) {
        if (error_message != nullptr) {
            *error_message = QStringLiteral("ASR PCM swr_convert failed: %1")
                                 .arg(FfmpegErrorString(converted_samples));
        }
        return false;
    }
...
bool PcmConverter::EnsureResampler(const AVFrame &frame,
                                   int input_channel_count,
                                   QString *error_message) {
    if (resampler_ != nullptr && input_sample_rate_ == frame.sample_rate &&
        input_format_ == frame.format && input_channel_count_ == input_channel_count) {
        return true;
    }
    ...
    if (alloc_result < 0) {
        if (error_message != nullptr) {
            *error_message = QStringLiteral("ASR PCM swr_alloc_set_opts2 failed: %1")
                                 .arg(FfmpegErrorString(alloc_result));
        }
        return false;
    }
